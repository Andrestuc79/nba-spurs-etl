...
WITH spurs_weaknesses_unpivoted AS (
    SELECT
        season2,
        weakness_type
    FROM {{ ref('team_weaknesses_unpivoted') }}
...
ranked_free_agents AS (
  SELECT
    ...
    FROM (select distinct player_id,player,position from {{ source('silver', 'players') }}) AS p
    JOIN (SELECT
    player_id,
    player_name,
    ...
    FROM {{ source('silver', 'player_stats') }} 
	group by player_id, player_name
    ) AS pgs
    ON p.player_id = pgs.player_id
    LEFT JOIN {{ source('silver', 'free_agents') }}  AS fa ON p.player_id = fa.player_id
    LEFT JOIN (Select player_id from {{ source('silver', 'injuries') }}  group by player_id) 
	AS i ON p.player_id = i.player_id
    LEFT JOIN (select player_id,max(salary_usd) as salary_usd from {{ source('silver', 'salaries') }} group by player_id) AS s ON p.player_id = s.player_id
),
